...
/**
 * Текст, содержащийся в одной строке страницы PDF-документа
 */
public class TextRow implements Comparable<TextRow> {
    private float x;
    ...
    public static TextRow newFor(TextPosition tp, PDGraphicsState gs) {
    ...
    public TextRow addBefore(TextRow t) {
        run = t.run + run;
        width += x - t.x + t.width;
    ...
    public TextRow addAfter(TextRow t) {
        run += t.run;
    ...
    public TextRow addBefore(TextPosition tp) {
    ...
    public TextRow addAfter(TextPosition tp) {
        run += tp.getCharacter();
    ...
    public boolean isIncidentToLeft(TextRow t) {
    ...
    public boolean isIncidentToRight(TextRow t) {
        return baseline == t.baseline
    ...
    public boolean hasMatchingStyle(TextRow t) {
        return t.font == font
                && t.strokeColor == strokeColor
                && t.nonStrokeColor == nonStrokeColor;
    }

    public int compareTo(TextRow other) {
        if (this.x < other.x) {
            return -1;
